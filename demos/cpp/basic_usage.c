/**
 * @file basic_usage.c
 * @brief Demonstrates basic SynaDB database operations in C
 * 
...
static int demo_crud(const char* db_path) {
    ...
    CHECK_ERROR(result, "SYNA_open");
    printf("  -> Database opened successfully\n");
    
    /* Write a float value */
    printf("\nWriting float value: temperature = 23.5\n");
    offset = SYNA_put_float(db_path, "temperature", 23.5);
    if (offset < 0) {
    ...
    offset = SYNA_put_int(db_path, "count", 42);
    if (offset < 0) {
        fprintf(stderr, "  -> Error: %s\n", error_to_string((int32_t)offset));
        return 1;
    }
    ...
    /* Write a text value */
    printf("\nWriting text value: message = \"Hello, SynaDB!\"\n");
    offset = SYNA_put_text(db_path, "message", "Hello, SynaDB!");
    if (offset < 0) {
        fprintf(stderr, "  -> Error: %s\n", error_to_string((int32_t)offset));
        return 1;
    }
    ...
    offset = SYNA_put_bytes(db_path, "binary_data", binary_data, sizeof(binary_data));
    if (offset < 0) {
        fprintf(stderr, "  -> Error: %s\n", error_to_string((int32_t)offset));
        return 1;
    }
    ...
    result = SYNA_get_float(db_path, "temperature", &temp_value);
    if (result == SYNA_SUCCESS) {
        printf("  -> Value: %f\n", temp_value);
    } else {
    ...
    result = SYNA_get_int(db_path, "count", &count_value);
    if (result == SYNA_SUCCESS) {
    ...
    char* text_value = SYNA_get_text(db_path, "message", &text_len);
    if (text_value != NULL) {
        printf("  -> Value: \"%s\" (length: %zu)\n", text_value, text_len);
        SYNA_free_text(text_value, text_len);
    } else {
    ...
    uint8_t* bytes_value = SYNA_get_bytes(db_path, "binary_data", &bytes_len);
    if (bytes_value != NULL) {
        printf("  -> Value: [");
        ...
        printf("] (length: %zu)\n", bytes_len);
        SYNA_free_bytes(bytes_value, bytes_len);
    } else {
        fprintf(stderr, "  -> Error: Key not found or type mismatch\n");
    }
    ...
    result = SYNA_exists(db_path, "temperature");
    printf("  -> Exists: %s\n", result == 1 ? "yes" : "no");
    
    printf("\nChecking if 'nonexistent' exists\n");
    result = SYNA_exists(db_path, "nonexistent");
    printf("  -> Exists: %s\n", result == 1 ? "yes" : "no");
    ...
    CHECK_ERROR(result, "SYNA_delete");
    printf("  -> Key deleted successfully\n");
    
    /* Verify deletion */
    printf("\nVerifying deletion - reading 'count':\n");
    result = SYNA_get_int(db_path, "count", &count_value);
    if (result == SYNA_ERR_KEY_NOT_FOUND) {
        printf("  -> Key not found (as expected after deletion)\n");
    } else if (result == SYNA_SUCCESS) {
    ...
    keys = SYNA_keys(db_path, &num_keys);
    if (keys != NULL) {
        for (size_t i = 0; i < num_keys; i++) {
            printf("  -> Key %zu: %s\n", i + 1, keys[i]);
        }
...
static int demo_timeseries(const char* db_path) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        int64_t offset = SYNA_put_float(db_path, "sensor/temp", value);
        if (offset < 0) {
    ...
    double* tensor = SYNA_get_history_tensor(db_path, "sensor/temp", &tensor_len);
    if (tensor != NULL) {
...
static int demo_error_handling(const char* db_path) {
    ...
    double value;
    result = SYNA_get_float(db_path, "test", &value);
    ...
    result = SYNA_get_float(db_path, "nonexistent_key", &value);
    printf("  -> Result: %d (%s)\n", result, error_to_string(result));
    
    /* Write an int, try to read as float (type mismatch) */
    printf("\nWriting int, then trying to read as float:\n");
    SYNA_put_int(db_path, "int_value", 100);
    result = SYNA_get_float(db_path, "int_value", &value);
    printf("  -> Result: %d (%s)\n", result, error_to_string(result));
    
    /* Try with NULL pointers */
    printf("\nTrying operations with NULL pointers:\n");
    result = SYNA_get_float(db_path, NULL, &value);
    printf("  -> get_float with NULL key: %d (%s)\n", result, error_to_string(result));
    
    result = SYNA_get_float(db_path, "test", NULL);
...
static int demo_compaction(const char* db_path) {
    ...
    for (int i = 1; i <= 5; i++) {
        SYNA_put_int(db_path, "counter", i * 10);
        printf("  -> Write %d: %d\n", i, i * 10);
    }
    ...
    double* tensor = SYNA_get_history_tensor(db_path, "counter", &tensor_len);
    if (tensor != NULL) {
    ...
    /* Compact database */
    printf("\nCompacting database...\n");
    result = SYNA_compact(db_path);
    CHECK_ERROR(result, "SYNA_compact");
    printf("  -> Compaction complete\n");
    
    /* Check history after compaction */
    printf("\nHistory after compaction:\n");
    tensor = SYNA_get_history_tensor(db_path, "counter", &tensor_len);
    if (tensor != NULL) {
...
int main(int argc, char* argv[]) {
    ...
    printf("SynaDB C Basic Usage Demo\n");
    printf("============================\n");
